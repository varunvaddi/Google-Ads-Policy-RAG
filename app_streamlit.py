...
<style>
    ...
    .decision-allowed { background-color: #d4edda; border-left: 4px solid #28a745; padding: 1rem; border-radius: 4px; }
    .decision-restricted { background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 1rem; border-radius: 4px; }
    .decision-disallowed { background-color: #f8d7da; border-left: 4px solid #dc3545; padding: 1rem; border-radius: 4px; }
...
def load_engine():
    """
    Load the RAG engine.
    Streamlit Cloud caches this object for performance.
    """
    ...
    if st.button("🔍 Review Ad", type="primary", use_container_width=True):
        if not ad_text:
            ...
            <div class="{decision_class}">
                <h2>{decision_emoji.get(decision.decision, '❓')} Decision: {decision.decision.upper()}</h2>
            </div>
            """, unsafe_allow_html=True)
...
with tab3:
    ...
    with col1:
        ...
        pipeline_data = {
            ...
            "Description": ["Offline (preprocessing)", "Offline (preprocessing)", "Dense + BM25 fusion",
                            "Cross-encoder scoring", "Gemini API call"]
        }
