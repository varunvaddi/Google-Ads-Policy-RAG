...
class PolicySearchV2Clean:
    """
    Search system using clean data (junk removed)
    """
    
    def __init__(self):
        embeddings_dir = Path("data/embeddings_v2")
        ...
        self.index = faiss.read_index(str(embeddings_dir / "faiss_clean.index"))
        
        # Load metadata (clean)
        with open(embeddings_dir / "metadata_clean.json", 'r') as f:
            self.metadata = json.load(f)
        
        # Load chunks (clean)
        with open('data/processed_v2/chunks_clean.json', 'r') as f:
